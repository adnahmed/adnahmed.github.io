...
export function ToolsAndInterests() {
  ...
  const parseEntireFile = useCallback((text: string) => {
    ...
    // Store all repos in ref
    allParsedRepos.current = parsedRepos;
    
    // Load initial set for the marquee
    const initial = parsedRepos.slice(0, INITIAL_LOAD_SIZE);
    setRepos(initial);
    nextRepoIndex.current = INITIAL_LOAD_SIZE;
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const nextIndex = nextRepoIndex.current;
      const totalAvailable = allParsedRepos.current.length;

      if (nextIndex < totalAvailable && repos.length < MAX_DOM_ITEMS) {
        const newBatch = allParsedRepos.current.slice(nextIndex, nextIndex + BATCH_SIZE);
        setRepos(prev => [...prev, ...newBatch]);
        nextRepoIndex.current = nextIndex + BATCH_SIZE;
      } else if (nextIndex >= totalAvailable || repos.length >= MAX_DOM_ITEMS) {
              ...
              {repos.map((repo, idx) => (
                  <a key={`${repo.url}-${idx}`} href={repo.url} target="_blank" rel="noopener noreferrer" className="mx-6 w-72 flex flex-col gap-2 p-4 rounded-lg bg-neutral-900/40 border border-white/[0.05] hover:border-white/[0.2] transition group">
                      <div className="flex items-center justify-between">
                          <span className="font-semibold text-neutral-200 truncate group-hover:text-blue-400 transition-colors max-w-[90%] text-sm">{repo.name}</span>
                          <FolderGit2 className="w-4 h-4 text-neutral-600" />
                      </div>
                      <p className="text-[11px] text-neutral-500 line-clamp-2 min-h-[2.5em] leading-relaxed">{repo.description}</p>
                      <div className="flex items-center gap-2 mt-auto pt-2 justify-between">
                           <span className="text-[9px] text-neutral-600 uppercase tracking-widest font-medium">GitHub</span>
                           <span className="text-[9px] px-1.5 py-0.5 bg-blue-900/30 text-blue-300 rounded border border-blue-800/50 font-medium">{repo.category}</span>
                      </div>
                  </a>
              ))}
           </Marquee>
